...
ssh -o StrictHosyKeyChecking=no ${TRAVIS_BRANCH} mkdir -p /opt/contentful-backend/${TRAVIS_BRANCH}
...
ssh -o StrictHostKeyChecking=no ${TRAVIS_BRANCH} cd /opt/contentful-backend/${TRAVIS_BRANCH} && yarn install
